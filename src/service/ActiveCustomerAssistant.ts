...
class ActiveCustomerAssistant implements CustomerAssistant<CardRegistrationDbo> {
  handle (registration: CardRegistrationDbo, inquiry: Inquiry): Promise<Array<Reaction>> {
    switch (inquiry.type) {
      case 'FREE_TEXT': {
        ...
        const intent: Optional<ActiveCustomerIntent> = TextExtractions.activeCustomerIntent(freeTextInquiry.freeText)
        if (!intent.isPresent()) {
      ...
      case 'SHOW_PARTNERS': {
        return this.handleIntent(ActiveCustomerIntent.SHOW_PARTNERS)
      }
      case 'SHOW_SUBSCRIPTIONS': {
        return this.handleIntent(ActiveCustomerIntent.SHOW_SUBSCRIPTIONS)
      }
      case 'SHOW_TUTORIAL': {
        return this.handleIntent(ActiveCustomerIntent.SHOW_TUTORIAL)
      }
      default: {
        return Results.many()
      }
      ...
      Reactions.choice(
        {
          ...
          title: StaticTexts.activeCustomerWelcome(
            Optional.ofNullable(registration.identification)
              .map((identification) => identification.firstName)
          ),
          subtitle: Optional.of(StaticTexts.activeCustomerIntentPrompt()),
          choices: [
  ...
  private handleIntent (intent: ActiveCustomerIntent): Promise<Array<Reaction>> {
    switch (intent) {
      case ActiveCustomerIntent.SHOW_TUTORIAL: {
      ...
      case ActiveCustomerIntent.SHOW_PARTNERS: {
        // TODO: from DB
        return Results.many(
          Reactions.choice(
            {
      ...
      case ActiveCustomerIntent.SHOW_SUBSCRIPTIONS: {
        // TODO: from DB
        return Results.many(
          Reactions.choice(
            {
...
export default ActiveCustomerAssistant
